...
void process_monkey(std::vector<monkey>& monkeys, int i, long long mod_base) {
    while (!monkeys[i].items.empty()) {
        ...
        // new_item /= 3; // Worry decreases since item not damaged. // Not anymore
        new_item %= mod_base;
...
int main() {
    ...
    long long mod_base {1};

    for (monkey monkey {}; std::cin >> monkey; ) {
        monkeys.push_back(monkey);
        mod_base *= monkey.test_mod;
    }
    ...
    for (int i {0}; i < 10000; i++) {
        for (int j {0}; j < static_cast<int>(monkeys.size()); j++) {
            process_monkey(monkeys, j, mod_base);
        }
    }

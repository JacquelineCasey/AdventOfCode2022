...
void compute_dir_to_delete(const directory_tree* dir, int space_needed, std::string& curr_dir_to_delete, int& dir_size) {
    if (dir->total_size >= space_needed && dir->total_size < dir_size) {
        dir_size = dir->total_size;
        curr_dir_to_delete = dir->name;
    }

    for (directory_tree* sub_dir : dir->sub_dirs) {
        compute_dir_to_delete(sub_dir, space_needed, curr_dir_to_delete, dir_size);
    }
}
...
int main() {
    ...
    int size_to_delete = root->total_size - 40000000;

    std::string curr_dir_to_delete {"/"};
    int dir_size = root->total_size;

    compute_dir_to_delete(root, size_to_delete, curr_dir_to_delete, dir_size);
